...
interface PostPreviewPanelProps {
  ...
  setSocialPlatforms: React.Dispatch<
    React.SetStateAction<Record<PlatformName, boolean>>
  >;
  setPostType: React.Dispatch<
    React.SetStateAction<{ post: boolean; story: boolean; reel: boolean }>
  >;
...
const PostPreviewPanel: React.FC<PostPreviewPanelProps> = ({
  ...
  const handlePostTypeClick = (type: "post" | "story" | "reel") => {
    setPostType((prevPostType) => ({
      ...
      story:
        type === "story" ? !prevPostType.story : prevPostType.story ?? false,
